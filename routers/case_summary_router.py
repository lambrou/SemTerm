...
async def create_summary(request: Request, case_data: CaseData):
        ...
        if case_data.source == 'user':
            if not case_data.summary:
                raise HTTPException(status_code=422,
                                    detail=f"Invalid input. Summary must be provided when source == user."
                                    )
            result = db['case_summaries'].insert_one(
                CaseSummary(summary=case_data.summary, source=case_data.source, case_data=case_data,
                            status='COMPLETED', completed_date=datetime.now()).dict()
            )
            return {"message": "Summary created.", "summary_id": str(result.inserted_id)}
        else:
            result = db['case_summaries'].insert_one(
    ...
    return {"message": "Summary creation in progress.", "summary_id": str(result.inserted_id)}
...
async def get_case_summary(request: Request, caseId: str) -> list[CaseSummaryResponse]:
    ...
    db = client['generative_summarizer']
    summary_list = await db['case_summaries'].find({"case_data.case_id": caseId}).sort(
