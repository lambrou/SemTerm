...
graphsignal.configure(
    api_key=settings.graphsignal_api_key, deployment=settings.environment
)
...
class CaseSummaryPipeline(BaseModel):
    ...
    def validate_model(cls, values):
        if not values.get("case_metadata") and not values.get("case_transcript"):
            return ValidationError(
                "Must have at least one of case_metadata or case_transcript"
            )
        else:
            return values
    ...
    def preprocess(self, data, type):
        unwanted_keys = [
            ...
            "owner_partners_teams_id",
            "source",
            "collaborator_partners_teams_ids",
            "type",
            "claimed",
            "created",
            "customer",
        ...
        if type == "transcript":
            ...
            for i, message in enumerate(data, 1):
                if message:
                    ...
                    if message["payload"] and "email" in message["payload"]:
                        email = message.get("payload").get("email").get(
                            "body"
                        ) or message.get("text")
            ...
            metadata_cleaned = {
                k: v for k, v in data.items() if v and k not in unwanted_keys
            }
    ...
    def process(self):
        ...
        summarizer = self.summary_chain_factory.create(
            SummaryInputType.CASE, SummaryChainType.REFINE
        )
        ...
        with graphsignal.start_trace("summary_pipeline"):
            ...
            if self.case_transcript and len(str(self.case_transcript)) > 50:
                ...
                summarized_transcript = summarizer.run(
                    {"input_documents": split_transcript}
                )

            if self.case_metadata and len(str(self.case_metadata)) > 100:
                split_metadata = self.preprocess(self.case_metadata, "metadata")
                summarized_metadata = summarizer.run(
                    {"input_documents": split_metadata}
                )

            if summarized_transcript and summarized_metadata:
                summary_summarizer = self.summary_chain_factory.create(
                    SummaryInputType.SUMM, SummaryChainType.REFINE
                )
                summarized_transcript_doc = [
                    Document(page_content=summarized_transcript)
                ]
                ...
                final_summary_surrogated = (
                    summarized_transcript
                    if summarized_transcript
                    else summarized_metadata
                )

        reidentified_summary = self.identity_handler.reidentify(
            final_summary_surrogated
        )
        return (
            reidentified_summary
            if final_summary_surrogated
            else "Not enough data for summary generation."
        )
